...
@section('content')
    <form action="{{ route('resumes.update', $resume) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input
                value="{{ old('desired_salary', $resume->desired_salary) }}"
        ...
        <div class="mb-3">
            ...
            <select name="employment_type_id" id="employment_type" class="form-select">
                @foreach($employmentTypes as $employmentType)
                    <option
                        {{ old('employment_type_id', $resume->employment_type_id) == $employmentType->id ? 'selected' : '' }}
            ...
            <select class="form-select" name="skills[]" id="skills" multiple>
                @foreach($skills as $skill)
                    <option value="{{ $skill->id }}"
                        {{ in_array(
                            $skill->id,
                            old('skills', $resume->skills->pluck('id')->toArray())
                        ) ? 'selected' : '' }}
                    >{{ $skill->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <textarea type="text" class="form-control" id="about" name="about" placeholder="Напишите что-нибудь о себе">{{ old('about', $resume->about) }}</textarea>
            @error('about')
            <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="resume_visibility_id" id="visibility" class="form-select">
                @foreach($visibilities as $visibility)
                    <option
                        {{ old('resume_visibility_id', $resume->visibility->id) == $visibility->id ? 'selected' : '' }}
